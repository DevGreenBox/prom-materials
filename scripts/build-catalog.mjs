...
function paramsOf(title) {
  ...
  const current = text.match(/(\d+(?:\.\d+)?)\s*(?:а|a|ампер)(?![а-яa-z0-9])/i);
  if (current) add("Номинальный ток", `${ru(current[1])} А`);

  const voltage = text.match(/(\d{2,4})\s*(?:v|в|вольт)(?:ac|dc|~)?(?![а-яa-z0-9])/i);
